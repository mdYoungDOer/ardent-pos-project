...
try {
    ...
    // Only super admins can manage knowledgebase
...
function createCategory($pdo, $data) {
    try {
        ...
        $checkStmt = $pdo->prepare("SELECT id FROM knowledgebase_categories WHERE slug = :slug");
        ...
        $sql = "INSERT INTO knowledgebase_categories (id, name, slug, description, icon, sort_order, created_at, updated_at) 
                VALUES (uuid_generate_v4(), :name, :slug, :description, :icon, :sort_order, NOW(), NOW()) RETURNING id";
...
function updateCategory($pdo, $data) {
    try {
        ...
        $checkStmt = $pdo->prepare("SELECT id FROM knowledgebase_categories WHERE slug = :slug AND id != :id");
        $checkStmt->execute(['slug' => $data['slug'], 'id' => $data['id']]);
        if ($checkStmt->fetch()) {
            http_response_code(400);
            echo json_encode(['success' => false, 'error' => 'Slug already exists']);
            return;
...
function createArticle($pdo, $data) {
    try {
        ...
        // Check for duplicate slug
        $checkStmt = $pdo->prepare("SELECT id FROM knowledgebase WHERE slug = :slug");
        $checkStmt->execute(['slug' => $data['slug']]);
        if ($checkStmt->fetch()) {
            http_response_code(400);
            echo json_encode(['success' => false, 'error' => 'Slug already exists']);
        ...
        $categoryStmt = $pdo->prepare("SELECT id FROM knowledgebase_categories WHERE id = :id");
        $categoryStmt->execute(['id' => $data['category_id']]);
        ...
        $sql = "INSERT INTO knowledgebase (id, category_id, title, slug, content, excerpt, tags, published, featured, view_count, created_at, updated_at) 
                VALUES (uuid_generate_v4(), :category_id, :title, :slug, :content, :excerpt, :tags, :published, :featured, 0, NOW(), NOW()) RETURNING id";
...
function updateArticle($pdo, $data) {
    try {
        ...
        $checkStmt = $pdo->prepare("SELECT id FROM knowledgebase WHERE slug = :slug AND id != :id");
        $checkStmt->execute(['slug' => $data['slug'], 'id' => $data['id']]);
        if ($checkStmt->fetch()) {
            http_response_code(400);
            echo json_encode(['success' => false, 'error' => 'Slug already exists']);
